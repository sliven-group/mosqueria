...
window.addEventListener('load', function () {
	const form = document.getElementById('mos-form-cb');
	const button = form.querySelector('#mos-form-cb-btn');
	const messageResult = document.getElementById('mos-form-cb-message');
	const departamentoSelect = document.getElementById('departamento-cb');
	const provinciaSelect = document.getElementById('provincia-cb');
	const distritoSelect = document.getElementById('distrito-cb');
	...
	const underAgeRadios = document.querySelectorAll('input[name="your-age-cb"]');
	const underAgeContainer = document.getElementById('pamatu-cb');
	const textarea = document.querySelectorAll('#mos-form-cb textarea');

	const validation = new JustValidate(form);
	const formData = new FormData();

	const validateDocNumber = (tipoDoc, value) => {
	...
	validation
		...
		.addField('#nro-doc-cb', [
			{
				validator: (value) => validateDocNumber(document.querySelector('#tipo-doc-cb').value, value),
		...
		.addRequiredGroup('#yourage_cb_radio_group')
		.addField('#tipo-rec-cb', [
			{
				rule: 'required',
				errorMessage: 'Campo requerido',
			},
		...
		.onSuccess(() => {
			...
			formData.append('flag_menor', document.querySelector('input[name="your-age-cb"]:checked')?.value ?? '');
			...
			formData.append('acepta_contenido', document.querySelector('input[name="acepto-cb"]:checked')?.value ?? '');
			formData.append('acepta_politica', document.querySelector('input[name="temr-cb"]:checked')?.value ?? '');
			...
			if (e.target.value === 'Si') {
				...
				validation
					...
					.addField('#padmatu-nro-doc-cb', [
						{
							validator: (value) => validateDocNumber(document.querySelector('#padmatu-tipo-doc-cb').value, value),
							errorMessage: 'Número de documentación inválido según el tipo',
						},
					]);
			} else {
	...
	const select = document.getElementById('tipo-doc-cb');
